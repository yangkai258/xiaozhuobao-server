...
const steps = ["DRAFT","SUBMITTED","INVITED","DONE"];
...
async function onSubmit() {
  ...
  if (!confirm) return;
  await run(async () => { await api_biz.create('MEETING', payload);
  ...
  <view class="page">
    ...
    <view class="dossier-cover" style="--mark-color: var(--c-accent)">
      <view class="cover-cust-row">
        <text class="cover-cust">会议推广登记</text>
      ...
      <view v-if="coverMetaVisible" class="cover-meta">
        <text class="mono">2026-07-25 14:00</text>
        <text class="sep" v-if="true">·</text>
        <text class="mono" v-if="true">工地会议室</text>
      </view>
    </view>
